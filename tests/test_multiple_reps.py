...
class RepModel(dp.Component):
    def __init__(self):
        ...
        curr_model = self
        arrival_cb = self.CustomerArrivalCallback(curr_model)
        ...
        abel_dist = dsr.get_empirical_pmf([2, 3, 4, 5], [0.3, 0.28, 0.25, 0.17],
                                          "Abel Service Distribution")
        baker_dist = dsr.get_empirical_pmf([3, 4, 5, 6], [0.35 ,0.25, 0.2, 0.2],
                                           "Baker Service Distribution")
        
        self.add_component("res_q", dp.ResourceQueue("Customer Servers"))
    ...
    class CustomerArrivalCallback(dp.AbstractCallback):
        def call(self):
            self.caller.res_q.request(dp.Entity("Customer"))
